...
let jcc_ipfs = {
  Ipfs: {
    ...
    async write(secret, data, filePath) {
      ...
      let encryptData = json5.stringify(
        JPassUtil.ECCCrypto.encryptWithPublicKey(publicKey, data)
      );
      let size = strlen(encryptData);
      let md5 = "md5";
      let timestamp = new Date().getTime();
      let sign = JPassUtil.Wallet.sign(
        privateKey,
        md5 + size + filePath + timestamp
      );
    ...
    async read(secret, filePath, address) {
      ...
      if (msg.indexOf("error") != -1) {
        ...
        return JPassUtil.ECCCrypto.decryptWithPrivateKey(
          privateKey,
          json5.parse(msg)
        );
      }

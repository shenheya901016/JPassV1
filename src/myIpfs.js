...
let myIpfs = {
  ...
  async read(userJID, userSecret) {
    ...
    for (let i = 0; i < jt_tokensOf.result.list.length; i++) {
      if (jt_tokensOf.result.list[i].symbol === "data") {
        return await this.readByHash(
          jt_tokensOf.result.list[i].token,
          userSecret
        );
      }
    }
  ...
  async readByHash(token, userSecret) {
    ...
    //使用私钥解密数据
    let msg = JPassUtil.ECCCrypto.decryptWithPrivateKey(
      key.privateKey,
      jt_getTokenByHash.result.Items[0].Value
    );
    return msg;
  },
  /**
   ...
   */
  async write(type, data, userJID, userSecret, operatorJID, operatorSecret) {
    //params:包括用户JID，运营商JID,用户JID私钥
    let jt_tokensOf = await remote.TokensOf([userJID]);
    if (type === "data") {
      for (let i = 0; i < jt_tokensOf.result.list.length; i++) {
        if (jt_tokensOf.result.list[i].symbol === "data") {
          ...
          let key = JPassUtil.Wallet.deriveKeyPair(userSecret);
          //使用公钥加密数据
          let encryptData = JPassUtil.ECCCrypto.encryptWithPublicKey(
            key.publicKey,
            data
          );
          return await this.getCreateToken(
            userJID,
            userSecret,
            operatorJID,
            operatorSecret,
            type,
            jt_tokensOf.result.list[i].info,
  ...
  async init(type, userJID, userSecret, operatorJID, operatorSecret) {
    let jt_issueToken = await this.getIssueToken(
      userJID,
      userSecret,
      operatorJID,
      operatorSecret,
    ...
    if (jt_issueToken.status === "success") {
      if (
        (await this.getTransactionStat(jt_issueToken.result[0].transaction)) ===
        "success"
      ) {
        let key = JPassUtil.Wallet.deriveKeyPair(userSecret);
        //使用公钥加密数据
